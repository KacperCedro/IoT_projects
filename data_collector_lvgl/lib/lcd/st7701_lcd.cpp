...
{   
   ...
   esp_lcd_panel_dev_config_t panel_config = {}; // empty struct to improve old code
    panel_config.reset_gpio_num = GPIO_NUM_5;
...
void st7701_lcd::fillScreen(uint16_t color)
{
    // Alokujemy pamięć tylko na JEDNĄ, pojedynczą linię ekranu (480 pikseli * 2 bajty)
    uint16_t *line_buffer = (uint16_t *)heap_caps_malloc(width() * 2, MALLOC_CAP_INTERNAL);
    
    if (line_buffer == NULL) {
        return; // Blokada bezpieczeństwa: unikamy crashu, jeśli nagle zabraknie pamięci
    }

    // Prawidłowe wpisanie 16-bitowego koloru do pamięci (zamiast destrukcyjnego memset)
    for (int i = 0; i < width(); i++) {
        line_buffer[i] = color;
    }

    // Malowanie ekranu linia po linii - ułamek zapotrzebowania na pamięć RAM
    for (int y = 0; y < height(); y++) {
        draw16bitbergbbitmap(0, y, width(), 1, line_buffer);
    }
